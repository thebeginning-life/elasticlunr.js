...
class Field {
  ...
  setToken(term, documents) {
    ...
    for (let [docId, options] of Object.entries(documents)) {
      ...
      if (ct < 0.01) {
        ...
        this._terms[term] && this._terms[term][docId] && delete this._terms[term][docId];
        this._tf[term] && this._tf[term][docId] && delete this._tf[term][docId];
  ...
  constructor(options) {
    ...
    this._terms = {}; // done
    this._documents = {};
    if (options.storeDocuments || options.storeDocuments === undefined) {
      this._store = true;
    }
    this._tf = {}; // done
    this._idf = {}; // done
  ...
  add(documents) {
    ...
    for (let o of documents) {
      ...
      tokens.forEach((token) => {
        ...
        if (!this._tf[term]) this._tf[term] = {};
        this._tf[term][o.id] = Math.sqrt(this._terms[term][o.id].total/tokens.length);
      });
    }
    this.recalculateIDF();
    return updated;
...
class Index extends IdxInterface {
  ...
  constructor(options) {
    ...
    this.expandFields = options.expandFields === undefined ? true : options.expandFields;
    this._fields[this._ref] = new Field({
      pipeline: {
        run: function (str) { return [str]; }
      }
    });
    this.documentStore = this;
    this.pipeline = options.pipeline;
  }
  size() {
    return this._fields[this._ref]._ids.length;
  }
  highlight(id, data) {
    // Pre-compute stuff
  ...
  getDoc(id) {
    let data = {};
    Object.entries(this._fields).forEach( ([name, field]) => { data[name] = field.get(id); });
    return data;
  }
  toJSON() {
    var fieldRef = [];
  ...
  add(documents, muteEvent) {
    ...
    documents.forEach((doc) => {
      ...
      var id = doc[this._ref];
      Object.entries(doc).forEach(([key, content]) => {
        if (this.expandFields && !this._fields[key]) {
          this.addField(key, {
            index: true,
            storePositions: true,
            storeDocuments: true,
  ...
  addField(field, options) {
    ...
    options = options || {};
    fieldOptions.index = options.index !== undefined ? options.index : true;
